...
# Modified   : Sunday December 17th 2023 04:40:38 pm                                               #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
class TestPlotImages:  # pragma: no cover
    ...
    # ============================================================================================ #
    def test_image_with_labels(self, images, plt):
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
        logger.info(double_line)
        # ---------------------------------------------------------------------------------------- #
        labels = self.get_texts(text="Image Label", n=4)
        plot_images(images=images, labels=labels, title="Fig Title")

        labels = self.get_texts(text="Image Label", n=3)
        with pytest.raises(ValueError):
