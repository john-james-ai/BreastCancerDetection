...
3. Add `mmg_id`, a composite identifier for each mammogram composed of abnormality type, fileset, patient id, left or right breast, and image view. 
4. Add a Boolean ‘cancer’ target variable that is True if the case is Malignant, False otherwise.
5. Remove the filepaths from the case dataset as they are not valid the study and series information will not be needed.
