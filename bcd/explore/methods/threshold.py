...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.12                                                                             #
# Filename   : /bcd/explore/methods/threshold.py                                                   #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
from bcd.preprocess.threshold import (
    Threshold,
...
class ThresholdAnalyzer:
    ...
    def _threshold_images(self, images: Tuple) -> None:
        ...
        for image in images:
            ...
            threshold, binary_mask = self._threshold.run(image=image)
            # Apply the mask
...
class ThresholdSurveyor:
    ...
    def tryall(self, image: np.ndarray) -> None:
        ...
        for idx, method in enumerate(self._methods.values(), 1):
            m = method()
            thresh, binary_mask = m.run(image)
